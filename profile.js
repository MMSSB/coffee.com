...
// Map of Drink Codes to Display Text
const DRINKS_MAP = {
    'tea': 'شاي في الخمسينة ☕',
...
function updateProfileInfo() {
    ...
    // Display Signature Drink next to Name
    if(nameEl) {
        let nameHTML = userData.fullName || 'الزبون';
        if (userData.signatureDrink && DRINKS_MAP[userData.signatureDrink]) {
            nameHTML += ` <span style="font-size: 0.8rem; background: #fff3cd; color: #856404; padding: 2px 8px; border-radius: 12px; vertical-align: middle; margin-right: 5px;">${DRINKS_MAP[userData.signatureDrink]}</span>`;
        }
